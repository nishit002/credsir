...
{{
  "links": [
    {{
      ...
      "target_url": "https://example.com/post-url",
...
def apply_internal_links_to_content(content, links_data):
    ...
    for link in links_data["links"]:
        ...
        # Create link replacement (only replace first occurrence)
        link_html = f'<a href="{target_url}" target="_blank">{anchor_text}</a>'
        modified_content = modified_content.replace(anchor_text, link_html, 1)
    
    return modified_content

def prepare_article_html(content, metadata):
    """Prepare article HTML with metadata"""
    # Add meta tags to content
    meta_tags = f"""
    <meta name="description" content="{metadata.get('meta_description', '')}" />
    <meta name="keywords" content="{metadata.get('primary_keyword', '')}, {', '.join(metadata.get('secondary_keywords', []))}" />
    """
    ...
    <html>
    <head>
        <title>{metadata.get('seo_title', 'Article')}</title>
        {meta_tags}
    </head>
    <body>
        {content}
...
def publish_to_wordpress_streamlined(title, content, metadata, image_buffer, wp_config, publish_now=True):
    """Streamlined WordPress publishing with error handling"""
    try:
        ...
        post_data = {
            ...
            "meta": {
                ...
                "_yoast_wpseo_focuskw": metadata.get('primary_keyword', ''),
                "_yoast_wpseo_title": metadata.get('seo_title', title)
            }
        }
        ...
        if image_buffer:
            try:
                ...
                image_buffer.seek(0)
                files = {'file': ('image.png', image_buffer.read(), 'image/png')}
                ...
                if media_response.status_code == 201:
                    ...
                    st.warning(f"Image upload failed: {media_response.status_code}")
            except Exception as e:
                st.warning(f"Image upload error: {str(e)}")
        
        # Create post
        posts_url = f"{wp_config['base_url']}/wp-json/wp/v2/posts"
        response = requests.post(posts_url, json=post_data, headers=headers)
        
        if response.status_code == 201:
            post_data = response.json()
            
            # Handle tags (create if don't exist)
            if metadata.get('tags'):
                try:
                    ...
                    for tag_name in metadata['tags']:
                        ...
                        # Check if tag exists
                        existing_tag = next((tag for tag in existing_tags if tag['name'].lower() == tag_name_lower), None)
                        ...
                        if existing_tag:
                            ...
                            tag_url = f"{wp_config['base_url']}/wp-json/wp/v2/tags"
                            tag_response = requests.post(tag_url, json=tag_data, headers=headers)
                            
                            if tag_response.status_code == 201:
                                new_tag = tag_response.json()
                                tag_ids.append(new_tag['id'])
                    
                    # Update post with tags
                    if tag_ids:
                        update_data = {"tags": tag_ids}
                        post_update_url = f"{posts_url}/{post_data['id']}"
                        requests.post(post_update_url, json=update_data, headers=headers)
                
                except Exception as e:
                    st.warning(f"Tag handling error: {str(e)}")
            
            return {
                "success": True,
                "title": metadata.get('seo_title', title),
                "url": post_data['link'],
                "has_image": featured_image_id is not None
            }
        else:
